...
export const AttachmentPage = {
  ...
  render: () => `
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:25px;">
            <div>
                <h2 style="margin:0; color:#fff;">Weapon Attachments</h2>
                <p style="color:#b9bbbe; margin:5px 0 0 0; font-size:0.9rem;">Modifikasi senjata Anda untuk performa maksimal.</p>
            </div>
        ...
        <div id="attachmentList" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 20px;">
  ...
  init: async (supabase) => {
    ...
    const loadAttachments = async () => {
      // 1. Ambil dari katalog_barang dengan jenis_barang 'Attachment'
      ...
      AttachmentPage.state.items = mergedItems;
      renderAttachments();
    };
          ...
          return `
            <div class="item-card" style="background: #2f3136; border-radius: 12px; padding: 20px; border: 1px solid #40444b; display: flex; flex-direction: column; transition: transform 0.2s;">
                ...
                <div style="flex-grow: 1;">
                    <h3 style="color: #fff; margin: 0 0 8px 0; font-size: 1.1rem; letter-spacing: 0.5px;">${item.nama_barang}</h3>
                    <p style="color: #b9bbbe; font-size: 0.8rem; line-height: 1.5; margin-bottom: 20px;">Scope, Grip, atau Muzzle untuk meningkatkan akurasi.</p>
                </div>
                
                <button 
                    onclick="window.addToCartAttach('${item.nama_barang}', ${item.harga_satuan}, ${item.stok})"
                    ${cannotBuy ? "disabled" : ""}
                    class="btn-order"
                    style="width: 100%; padding: 12px; border-radius: 8px; border: none; font-weight: 800; font-size: 0.85rem; cursor: ${cannotBuy ? "not-allowed" : "pointer"}; 
                    background: ${cannotBuy ? "#4f545c" : "#5865f2"}; color: white; transition: 0.3s; text-transform: uppercase;">
                    ${
                      isNotReady 
                      ? "Not Ready" 
                      : (isOut ? "Out of Stock" : '<i class="fas fa-cart-plus" style="margin-right:8px;"></i> Ambil Attach')
                    }
                </button>
            </div>
        `;
    ...
    window.addToCartAttach = (name, rawPrice, stock) => {
      if (stock <= 0) return;
