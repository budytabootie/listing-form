...
export const StokWeaponPage = {
  ...
  render: () => `
    ...
    <div style="background: #2f3136; padding: 25px; border-radius: 12px; margin-bottom: 25px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); border-top: 4px solid #5865F2;">
        ...
        <div style="display: grid; grid-template-columns: 1.5fr 1.5fr auto; gap: 15px; align-items: end; margin-top:15px;">
            ...
            <button id="btnSaveWeapon" style="background:#5865F2; padding: 0 30px; font-weight:bold; height:42px; border-radius:5px; cursor:pointer; border:none; color:white;">REGISTRASI</button>
        </div>
    </div>
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; gap: 15px;">
         <div style="position:relative; flex: 2;">
            ...
            <input type="text" id="weapSearch" placeholder="Cari Nama Senjata atau SN..." 
         ...
         <select id="weapFilterStatus" style="flex: 1; background:#202225; border:1px solid #4f545c; border-radius:8px; color:white; padding:10px;">
            ...
            <option value="broken">Bermasalah (Broken/Lost/Seized)</option>
         </select>
    </div>
        ...
        <table style="width: 100%; border-collapse: collapse; font-size:0.9rem;">
            <thead>
                <tr style="background: #202225; text-align: left; color:#b9bbbe;">
                    <th style="padding: 18px; cursor: pointer;" id="sortWeapon">NAMA ITEM <i class="fas fa-sort fa-xs"></i></th>
                    <th style="padding: 18px; cursor: pointer;" id="sortSN">SN <i class="fas fa-sort fa-xs"></i></th>
                    <th style="padding: 18px;">STATUS</th> 
                    <th style="padding: 18px; cursor: pointer;" id="sortHold">PEMEGANG <i class="fas fa-sort fa-xs"></i></th> 
  ...
  init: async (supabase) => {
    ...
    const loadWeapons = async () => {
      ...
      let filtered = data.filter((item) => {
        ...
        if (st.statusFilter === "available") {
          ...
          matchesFilter =
            !!item.hold_by &&
            (currentStatus === "available" || currentStatus === "in_use");
    ...
    const renderSummary = (allData) => {
      const summary = {};
      allData.forEach((w) => {
        if (!w.hold_by && (w.status === "available" || !w.status)) {
      ...
      container.innerHTML =
        entries.length === 0
          ? `<div style="color:#72767d; font-size:0.8rem;">Gudang kosong...</div>`
          : entries
              .map(
                ([n, q]) =>
                  `<div style="background:#23272a; padding:12px; border-radius:10px; border-left:4px solid #5865F2; display:flex; justify-content:space-between; align-items:center;"><span style="font-size:0.75rem; font-weight:bold; color:#fff;">${n}</span><span style="background:#5865F2; color:white; padding:2px 8px; border-radius:20px; font-size:0.7rem;">${q}</span></div>`
              )
              .join("");
    };
    ...
    const renderTable = (items) => {
      ...
      const getStatusBadge = (status, hold) => {
        if (status === "broken")
          return `<span style="color:#ed4245; border:1px solid #ed4245; padding:2px 8px; border-radius:20px; font-size:0.65rem; font-weight:bold;">BROKEN</span>`;
        if (status === "lost")
          return `<span style="color:#72767d; border:1px solid #72767d; padding:2px 8px; border-radius:20px; font-size:0.65rem; font-weight:bold;">LOST</span>`;
        if (status === "seized")
          return `<span style="color:#faa61a; border:1px solid #faa61a; padding:2px 8px; border-radius:20px; font-size:0.65rem; font-weight:bold;">SEIZED</span>`;
        return hold
          ? `<span style="color:#5865F2; border:1px solid #5865F2; padding:2px 8px; border-radius:20px; font-size:0.65rem; font-weight:bold;">IN USE</span>`
          : `<span style="color:#43b581; border:1px solid #43b581; padding:2px 8px; border-radius:20px; font-size:0.65rem; font-weight:bold;">GUDANG</span>`;
      };
      ...
      tableBody.innerHTML = items
        ...
        <tr style="border-bottom: 1px solid #36393f; transition:0.2s;" onmouseover="this.style.background='#36393f'" onmouseout="this.style.background='transparent'">
            ...
            <td style="padding: 15px;">${getStatusBadge(
              w.status,
              w.hold_by
            )}</td>
            <td style="padding: 15px; color:${w.hold_by ? "#fff" : "#4f545c"};">
                ${
                  w.hold_by
                    ? `<i class="fas fa-user-circle" style="margin-right:5px; color:#5865F2;"></i>${w.hold_by}`
                    : '<span style="opacity:0.5; font-style:italic;">In Storage</span>'
                }
            </td>
            ...
            w.status || "available"
          }" style="background:#faa61a; padding:6px 12px; margin-right:5px; border-radius:4px; border:none; color:white; cursor:pointer;"><i class="fas fa-edit"></i></button>
      ...
      tableBody.querySelectorAll(".btn-edit-weap").forEach((btn) => {
        btn.onclick = async () => {
          const { id, hold, status } = btn.dataset;
          const { value: formValues } = await Swal.fire({
            title: "Update Weapon Info",
            background: "#2f3136",
            color: "#fff",
            html: `
                <div style="text-align:left; font-size:0.75rem; color:#b9bbbe; margin-bottom:5px;">PEMEGANG</div>
                <select id="swal-hold" class="swal2-input" style="width:100%; margin:0 0 15px 0; background:#202225; color:white;">
                    <option value="">-- KEMBALIKAN KE GUDANG --</option>
                    ${listMembers
                      .map(
                        (m) =>
                          `<option value="${m.nama}" ${
                            hold === m.nama ? "selected" : ""
                          }>${m.nama}</option>`
                      )
                      .join("")}
                </select>
                <div style="text-align:left; font-size:0.75rem; color:#b9bbbe; margin-bottom:5px;">KONDISI</div>
                <select id="swal-status" class="swal2-input" style="width:100%; margin:0; background:#202225; color:white;">
                    <option value="available" ${
                      status === "available" || status === "in_use"
                        ? "selected"
                        : ""
                    }>Good / Available</option>
                    <option value="broken" ${
                      status === "broken" ? "selected" : ""
                    }>Broken (Rusak)</option>
                    <option value="lost" ${
                      status === "lost" ? "selected" : ""
                    }>Lost (Hilang)</option>
                    <option value="seized" ${
                      status === "seized" ? "selected" : ""
                    }>Seized (Disita)</option>
                </select>`,
            preConfirm: () => {
              const h = document.getElementById("swal-hold").value || null;
              const s = document.getElementById("swal-status").value;
              // LOGIKA OTOMATIS: Jika Good dan ada pemegang, set 'in_use'
              let finalStatus = s;
              if (s === "available" && h) finalStatus = "in_use";
              if (s === "available" && !h) finalStatus = "available";
              return { hold_by: h, status: finalStatus };
            },
          });
          if (formValues) {
            await supabase
    ...
    document.getElementById("sortWeapon").onclick = () => {
      st.sortField = "weapon_name";
      st.sortAsc = !st.sortAsc;
      loadWeapons();
    };
    document.getElementById("sortSN").onclick = () => {
      st.sortField = "serial_number";
      st.sortAsc = !st.sortAsc;
      loadWeapons();
    };
    document.getElementById("sortHold").onclick = () => {
      st.sortField = "hold_by";
      st.sortAsc = !st.sortAsc;
      loadWeapons();
    };
